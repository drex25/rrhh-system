...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex justify-between items-center mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Editar Entrevista</h1>
                <p class="mt-2 text-gray-600 dark:text-gray-400">Modifique los detalles de la entrevista programada</p>
            </div>
            <div class="flex space-x-4">
                <a href="{{ route('interviews.show', $interview) }}" class="inline-flex items-center px-4 py-2 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-white rounded-lg transition-colors duration-200 border border-gray-200 dark:border-gray-700">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
            <form action="{{ route('interviews.update', $interview) }}" method="POST" class="space-y-8 p-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="job_posting_id" id="job_posting_id" required
                                ...
                                @foreach($jobPostings as $jobPosting)
                                    <option value="{{ $jobPosting->id }}" {{ old('job_posting_id', $interview->job_posting_id) == $jobPosting->id ? 'selected' : '' }}>
                                        {{ $jobPosting->title }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="type" id="type" required
                                ...
                                <option value="phone" {{ old('type', $interview->type) == 'phone' ? 'selected' : '' }}>Teléfono</option>
                                <option value="video" {{ old('type', $interview->type) == 'video' ? 'selected' : '' }}>Video</option>
                                <option value="in_person" {{ old('type', $interview->type) == 'in_person' ? 'selected' : '' }}>Presencial</option>
                            </select>
                            @error('type')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2" id="location_field" style="display: {{ $interview->type === 'in_person' ? 'block' : 'none' }};">
                        ...
                        <div class="space-y-2" id="meeting_link_field" style="display: {{ $interview->type === 'video' ? 'block' : 'none' }};">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="notes" id="notes" rows="4"
                            class="w-full px-4 py-3 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('notes', $interview->notes) }}</textarea>
                        @error('notes')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
